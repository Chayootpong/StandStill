...
public class Attack : MonoBehaviour {
    ...
    bool isCheck = true, isTurnLeft = true, isLeft = true;
    ...
    void Update() {
        ...
        {
            if (isLeft && isHold == 1) { gladiator[3].SetActive(true); gladiator[4].SetActive(false); }
            else if (!isLeft && isHold == 2) { gladiator[3].SetActive(false); gladiator[4].SetActive(true); }
    ...
    {
        isTurnLeft = true; isLeft = true;
        AttackTarget();
    }

    public void RightAttack()
    {
        isTurnLeft = false; isLeft = false;
        AttackTarget();
    }

    public void OnPointerDown(int side)
    {
        isHold = side;
        if (isHold == 1) { isTurnLeft = true; isLeft = true; }
        else if (isHold == 2) { isTurnLeft = false; isLeft = false; }
    }
